...
function Messages() {
    ...
    const fetchNotifications = async () => {
        try {
            setLoading(true);
            const response = await api.get('/driver/notifications/all', {
    ...
    const deleteSelectedMessages = async () => {
        ...
        try {
            for (const id of selectedIds) {
                await api.delete('/driver/notifications/delete', {
    ...
    const markSelectedMessages = async () => {
        ...
        try {
            for (const id of selectedIds) {
                const response = await api.put('/driver/notifications/mark-as-read',
    ...
    const deleteAllMessages = async () => {
        try {
            const response = await api.delete('/driver/notifications/delete-all', {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if(response.status === 200){
                console.log("All Messages are deleted successfully");
                setNotifications([]);
            }
        } catch (err) {
    ...
    const markAllMessages = async () => {
        try {
            const response = await api.put('/driver/notifications/mark-all-as-read', {}, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if(response.status === 200){
                console.log("All Messages are mark as read successfully");
