...
const Settings = () => {
  ...
  //
  // const goToAccount = () => {
  //   navigate("/Account"); // navigate to DriverAccount page
  //
  // };
  //
  const goToPrivacy =() => {
    navigate("/Privacy"); // navigate to DriverPrivacy page
  };
  //
  const goToAbout =() => {
    navigate("/About"); // navigate to DriverAbout page
  };
  //
  // return (
  //
  ...
  //     {/* Log Out */}
  //     <button
  //       className="btn btn-light text-danger mt-4 w-100 d-flex align-items-center justify-content-center"
  //       style={{
  ...
  const DeleteAccount = async () =>{
    try {
      const response = await api.delete('/driver/delete-account', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
  ...
  const handleChangeUsername = async () => {
    ...
    try {
      const response = await api.post('/driver/changer-username', {
        username:username,
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleChangePassword = async () => {
    ...
    try {
      const response = await api.post('/driver/change-password', {
                ...
                <div className="card-body bg-white rounded-top-4 rounded-bottom-4">
                  <ul className="list-group list-group-flush">
                    ...
                    <SettingItem icon={<FaQuestionCircle />} label="Help & Support" bar id="support"
                                 onClick={setActiveSection} />
                    <SettingItem icon={<FaShieldAlt />} label="Privacy" bar id="privacy"
                                 onClick={goToPrivacy} />
                    <SettingItem icon={<FaInfoCircle />} label="About" bar id="about"
                                 onClick={goToAbout} />
